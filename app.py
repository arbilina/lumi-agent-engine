...
DASHBOARD_BASE_URL = "https://your-lumi-dashboard.com/protocol" 
...
def get_lumi_supplement_stack(user_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    meds = [m.lower() for m in user_data.get('medications', [])]
    conditions = [c.lower() for c in user_data.get('conditions', [])]
    ...
    diet_notes = lifestyle.get('diet_notes', '').lower() 
    movement = lifestyle.get('movement', 'sedentary').lower() 
    ...
    # Safety Checks
    if any(m in meds for m in ['ssri', 'antidepressant', 'zoloft', 'prozac']):
        warnings.append("RISK: St. John's Wort is contraindicated with SSRIs and has been excluded.")
    if any('thyroid' in m for m in meds):
    ...
    if any(c in conditions for c in ['liver disorder', 'liver disease']):
        warnings.append("RISK: Black Cohosh is avoided due to history of liver disorder.")
    if any(c in conditions for c in ['estrogen-sensitive', 'breast cancer']):
        warnings.append("RISK: Phytoestrogens (like Soy, Red Clover) are avoided due to your medical history.")
    if not any(c in conditions for c in ['iron deficiency', 'anemia']):
    ...
    final_recommendation = {
        ...
        "rationale_summary": f"Your plan was built using evidence-backed clusters, targeting your key symptoms and refined based on your **{sleep_quality}** sleep and **{stress_level}/10** stress level.",
        "daily_plan": [
...
def handle_get_protocol():
    try:
        ...
        if not user_data or 'user_id' not in user_data or 'symptoms' not in user_data:
            return jsonify({"error": "Missing required data: user_id and symptoms are required."}), 400
        
        protocol = get_lumi_supplement_stack(user_data)
        return jsonify(protocol), 200
        
    except Exception as e:
        print(f"Error in /api/get-protocol: {e}")
